...
public class blockPageRank {
	...
	public static void main(String[] args) throws Exception{
		...
		int flag = 0;
		do{
			Job job = blockPageRank.getMapReduceJob();
			if (flag == 0){
		...
		HashMap<Integer,Float> sample = blockPageRank_reduce.getSampleNodes();
		...
		int blockID = 0;
		for(Integer j : keys){
			if(blockPageRank_map.checkInblocks(j)){
	...
	public static Job getMapReduceJob() throws IOException{
		...
		job.setMapperClass(blockPageRank_map.class);
		job.setReducerClass(blockPageRank_reduce.class);		
		return job;	
	}
}
