...
public class randomBlockPartition_map extends Mapper<LongWritable, Text, Text, Text>{
	...
	public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException{
		...
		int blockID = badBlockIDofNode(nodeID);
		...
		if (outgoingLinks != "") {	
			...
			for(String s : outlinks){
				int blockIDofNeighbor = badBlockIDofNode(Integer.parseInt(s));
	...
	public int badBlockIDofNode(int nodeID){
		return (nodeID % 68);		
	}
}