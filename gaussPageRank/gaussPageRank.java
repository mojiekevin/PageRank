...
public class gaussPageRank {
	...
	public static void main(String[] args) throws Exception{
		...
		int flag = 0;
		do{
			Job job = gaussPageRank.getMapReduceJob();
			if (flag == 0){
			...
			job.getCounters().findCounter(accumulator.counter.ITERATION).setValue(0);
			flag++;
		}while(avgResidualError > threshold);
		//Entire computation has converged, report the sample of PageRank values.
        
		HashMap<Integer,Float> sample = gaussPageRank_reduce.getSampleNodes();
		...
		int blockID = 0;
		for(Integer j : keys){
			if(gaussPageRank_map.checkInblocks(j)){
	...
	public static Job getMapReduceJob() throws IOException{
		...
		job.setMapperClass(gaussPageRank_map.class);
		job.setReducerClass(gaussPageRank_reduce.class);		
		return job;	
	}
}
