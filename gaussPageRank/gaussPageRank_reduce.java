...
public class gaussPageRank_reduce extends Reducer<Text, Text, Text, Text> {
	...
	public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException{
		...
		do {
			...
			flag++;
		} while (residualError > gaussPageRank_reduce.stoppingThreshold);
	...
	public float IterateBlockOnce(){
		...
		for(String k : records.keySet()){
			...
			currPageRank = gaussPageRank_reduce.randomJump*currPageRank+((float)(1-gaussPageRank_reduce.randomJump))/gaussPageRank.numOfwebGraphNodes;
		...
		for(String k : records.keySet()){
			res_error += Math.abs(oldPR.get(k)-updatedPageRank.get(k)) / updatedPageRank.get(k);
		}
